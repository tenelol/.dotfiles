...
let
  baseConfig =
    if pkgs.stdenv.hostPlatform.isDarwin then ./ghostty/files/config-darwin else ./ghostty/files/config;
...
delib.module {
  ...
  home.ifEnabled =
    ...
    {
      xdg.configFile = {
        "ghostty/config".source = mkConfig myconfig;
        "ghostty/shaders/aurora.glsl".source = auroraShader;
        "ghostty/shaders/liquid_glass_focus.glsl".source = ./ghostty/files/shaders/liquid_glass_focus.glsl;
        "ghostty/shaders/readability_scrim.glsl".source = mkReadabilityShader myconfig;
        "ghostty/shaders/cursor_tail.glsl".source = cursorTailShader;
        "ghostty/shaders/ripple_rectangle_cursor.glsl".source = rippleRectangleCursorShader;
      };
    };
}
