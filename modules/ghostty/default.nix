...
let
  baseConfig =
    if pkgs.stdenv.hostPlatform.isDarwin then ./files/config-darwin else ./files/config;
  ...
  home.ifEnabled =
    { myconfig, ... }:
    {
      xdg.configFile = {
        ...
        "ghostty/shaders/liquid_glass_focus.glsl".source = ./files/shaders/liquid_glass_focus.glsl;
