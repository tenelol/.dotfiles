import { InsertExitChord } from "./insert-exit-chord.js";
import {
  CustomEditor,
  type ExtensionAPI,
...
export class ModalEditor extends CustomEditor {
  ...
  /** Use the footer status instead of an editor-border badge when false. */
  showModeLabel = true;
  ...
  private readonly insertExitChord = new InsertExitChord(
    (data) => {
      try {
    ...
    () => this.mode === "insert" && this.pendingExCommand === null && !this.replayingRepeat,
  );

  handleInput(data: string): void {
    this.insertExitChord.handle(data);
  }
  ...
  private handleEscape(): void {
    ...
    // In Normal mode Esc is a Vim no-op, not an agent interrupt.
  }
  ...
  render(width: number): string[] {
    ...
    if (lines.length === 0 || !this.showModeLabel) return lines;
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    ctx.ui.setEditorComponent((tui, theme, kb) => {
      ...
      // Match pi-vim-flash's plain footer status; keep Vim editing unchanged.
      editor.showModeLabel = false;
      const updateModeStatus = (mode: Mode) => {
        ctx.ui.setStatus("vim-mode", mode === "visual-line" ? "VISUAL LINE" : mode.toUpperCase());
      };
      updateModeStatus(editor.getMode());
      editor.setModeChangeFn((mode, prevMode) => {
        updateModeStatus(mode);
        modeChangeHandler(mode, prevMode);
      });
