...
6. Route delegated packets through `$subagent-model-router` when it is available, then dispatch only when the current environment allows it and the user has authorized delegated or parallel agent work.
...
Ownership:
Routing tier:
Resolved model / reasoning / backend:
Do:
Do not:
Expected output:
Verification:
...
Before dispatch, use `$subagent-model-router` when it is available:

1. Assign each packet a `fast`, `standard`, `deep`, or `review` tier.
2. Let the router resolve the live model, reasoning effort, and native or CLI backend.
3. Record the resolved values in the packet and workflow state before launch.
4. Keep packet planning, approvals, integration, conflict resolution, and verification in this skill.

Do not duplicate model IDs or CLI invocation rules here. If the router is unavailable, use the available native runner with inherited configuration and record that explicit model routing was not applied.

When a native or routed subagent runner is available:
...
- For a router CLI backend, treat the worker as an independent leaf process, store its final output under `results/`, and do not expect native follow-up or wait tools.

When neither a native runner nor the router CLI backend is available:
