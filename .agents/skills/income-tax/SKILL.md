...
- 基礎控除の額は源泉徴収票に記載があればその値を使う。未記載の場合は合計所得と年分に応じて算出する（令和7年分: 58万+加算で最大95万、令和8・9年分: 62万+加算で最大104万、令和10年分以後: 62万+加算は132万以下のみ99万）
...
4. 公的年金等控除の最低保障額（65歳未満60万、65歳以上110万）は令和7年度・8年度改正でも据え置き。令和9年分以後は給与所得控除との合計が280万を超える場合に超過分を年金控除から減額（令和8年度改正、計算未反映）
...
- 基礎控除: 合計所得金額に応じた段階的控除（令和7年分: 132万以下=95万、令和8年分: 489万以下=104万。`fiscal_year` で自動切替）
