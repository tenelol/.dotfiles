...
令和7年分（2025年）・令和8年分（2026年）対応。令和7年10月のポイント付与禁止、令和8年10月の指定基準見直しを反映。
...
※ 上記は令和7年分の概算値（総務省の目安表ベース、社会保険料は給与収入の15%と仮定）。令和8年分は基礎控除の引上げ（最大104万）で所得税の課税所得が税率境界を下回る層（例: 給与500万・独身は10%→5%で約57,500円）の上限が下がる。住宅ローン控除・iDeCo・医療費控除等がある場合は減少する。事業所得がある場合は別途計算が必要。
...
正確な上限額は `shinkoku tax calc-furusato-limit`（住民税側の控除・調整控除を考慮した詳細推定）で計算し、`shinkoku furusato summary --estimated-limit N` で寄附合計と比較する。
...
総務省の告示により、対象自治体は以下の基準を満たす必要がある:

- **返礼割合**: 寄附額の**30%以下**（令和元年6月〜）
- **募集費用**: 返礼品を含む募集費用の合計が寄附額の**50%以下**（令和元年6月〜）。令和8年10月1日以後の指定からは「寄附金活用可能額」が寄附額の**60%以上**（経過措置: R8.10〜52.5%、R9.10〜55%、R10.10〜57.5%）
- **地場産品基準**: 当該地方団体の区域内で生産・製造・加工等されたもの。令和8年10月以後は広報目的の例外（キャラクターグッズ等）が直近1年の配布・販売実績数量までに限定され、加工品は区域内で5割超の付加価値が必要
- 基準を超える自治体は「ふるさと納税の対象団体」から除外される

### ポイント付与の禁止（令和7年10月1日〜）

ポータルサイト等が寄附に伴ってポイント等を付与することは禁止された（令和6年6月総務省告示改正、令和7年10月1日施行）。
クレジットカード会社が決済に対して付与する通常ポイントは対象外。
