...
特例控除（③）の計算における「所得税率」は、住民税の課税総所得金額から**人的控除差調整額**と**（所得税の基礎控除額 − 48万円）**を差し引いた金額で判定する（地方税法37条の2第2項。後者は令和7年分の基礎控除引上げに伴う措置で令和8年度分住民税から適用）。この結果、判定基準は所得税の課税所得と一致し、所得税の限界税率と整合する。

```
判定基準 = 住民税課税総所得金額 − 人的控除差の合計 − (所得税の基礎控除額 − 48万円)

人的控除差調整額の例（調整控除と共通）:
  基礎控除: 一律5万円（所得税の基礎控除の引上げ分は上記の別項で控除する）
...
## 令和8年度税制改正（令和8年分以後）の影響

### ふるさと納税そのものの改正

| 項目 | 内容 | 適用時期 |
|------|------|---------|
| 特例控除額の上限 | 住民税所得割額の2割と193万円（道府県民税77.2万+市町村民税115.8万）のいずれか低い方 | 令和10年度分住民税（令和9年中の寄附）以後 |
| 指定基準の見直し（6割ルール） | 寄附金活用可能額 ≥ 寄附額の60%（R8.10〜52.5%、R9.10〜55%、R10.10〜57.5% の経過措置） | 令和8年10月1日以後の指定 |
| 地場産品基準の厳格化 | 広報目的の例外を直近1年の配布・販売実績数量まで等に限定 | 令和8年10月1日以後の指定 |
| ポイント付与の禁止 | ポータルサイト等による寄附に伴うポイント等の付与を禁止 | 令和7年10月1日以後 |

### 令和8年分の所得税改正とふるさと納税上限額

令和8年分から所得税の基礎控除が最大104万円（合計所得489万円以下）、給与所得控除の最低保障額が74万円になった。

- **住民税の基礎控除は43万円で据え置き**（令和9年度分も変更なし）。上限額の元になる住民税所得割額は変わらない
- ただし特例控除の所得税率は所得税の課税所得で判定されるため、基礎控除の引上げで所得税の課税所得が税率の境界（195万・330万等）を下回る層は、税率が下がり上限額も下がる（例: 給与収入500万・独身・社保15%: 令和7年分 約61,000円 → 令和8年分 約57,500円。所得税率10%→5%）
- 所得税の課税所得から住民税所得割を逆算する簡易推定は、基礎控除差（最大61万円 → 所得割6.1万円）の分だけ所得割を過少に見積もる。`shinkoku tax calc-furusato-limit` の詳細推定（住民税側の控除で計算）を使う
- 給与収入190万〜220万円の層は給与所得控除が最大9万円増えるため、住民税所得割が最大9,000円減り、上限額が2,000〜3,000円程度下がる
