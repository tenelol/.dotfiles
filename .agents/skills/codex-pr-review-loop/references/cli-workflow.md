...
子エージェントを使わないという指定はCLIレビュアーにも適用する。その場合は起動せず、許可された親のレビューと修正を行い、独立レビュー未実施を明示する。
修正まで依頼されている場合は、確認済み・scope内の指摘を親がトリアージして修正する。各ラウンドで再承認を要求しない。レビューだけの依頼ではコード・計画を変更しない。
既定はSol / `xhigh`で、Lunaは選択しない。機密情報や外部操作の許可を子へ勝手に追加しない。
...
- すでにPRのブランチにいる場合はそのままでよい。無関係な未コミット変更がある場合はそれを保持し、必要なら別worktreeを使う。安全に分離できない変更だけ扱いを確認する

### 2. 開始

レビュアーはSol / `xhigh`を既定にする。モデルと推論強度はスクリプトが明示し、既存の `codexExtraArgs` によるSol/Terraおよび明示された `max` は保持する。Lunaは使わない。通常は一時設定ファイルを作らず、既存のproject設定をそのまま読む。

```bash
python3 "${CLAUDE_SKILL_DIR}/scripts/pr_review.py" start --backend codex --pr "<PRのURL>"
```
...
| nextAction | やること |
| --- | --- |
| `triage` | 明示的に人間のトリアージを要求する設定がある場合だけ、要点と判断が必要な指摘を提示する（手順5へ） |
...
| レビューが長時間無応答 | ログで進捗と終了状態を確認する。終了していない同じ処理を重複起動しない。モデル・effortは黙って変更せず、足りない文脈や具体的な失敗を調べる。 |
