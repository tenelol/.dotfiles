...
情報は令和7年分（2025年）および令和8年分（2026年、令和8年度税制改正）の税制に基づいています。
```
...
- 金額・税率は令和7年分（2025年）・令和8年分（2026年）の値を年分に応じて使用している

### 情報の限界

以下の点について利用者に注意を促す:

1. **法改正への追従**: 税制は毎年改正される。令和9年分以降の税制（防衛特別所得税・復興特別所得税1.1%、青色申告特別控除65万/75万、ひとり親控除38万、公的年金等控除の280万上限、基礎控除の物価連動見直し等）は定数のみ用意しており、計算には未反映。新たな改正に基づく情報を確認する必要がある
