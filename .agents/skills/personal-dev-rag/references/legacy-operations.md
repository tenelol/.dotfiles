...
Use these operations only for explicitly requested old-RAG inspection, recovery, migration or forgetting. Normal durable context follows the current project-context protocol. Do not reconnect disabled legacy services as part of unrelated work.
...
If MCP tools are unavailable, check the current configuration; legacy services need not be enabled for normal project-context work. Do not try to restore or reconnect it unless the user explicitly asks to recover, migrate, forget, or inspect old RAG content.
...
- For normal Codex work, do not use these retrieval rules; use the current project-context protocol only when prior context is needed.
...
Keep old Notion and personalDevRag content intact unless its migration or deletion is explicitly requested. When legacy evidence is useful, verify it against current primary sources before recording a concise conclusion in the current project context. Do not reinstate Notion-first or RAG-first hooks.
