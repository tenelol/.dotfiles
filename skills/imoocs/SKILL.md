...
Audit every blank visible field before `push`; do not treat a conditional field as safely complete merely because its condition is false. When an inapplicable field is free text and the user asked to complete the whole form, enter a concise explicit value such as `該当なし（交互作用効果なし）` unless the form explicitly requires the field to remain blank. When the user wants every visible control completed, also solve conditional radio/select branches whenever the underlying analysis supports one of their choices, select that choice, and preserve any interpretation caveat in the notebook or explanation. Leave a visible control unselected only when no available choice can be supported without guessing or contradiction, and report the exact field and reason before `push`.
