...
- 外部ディスプレイ運用では cursor warp と scrolling animation を抑えて安定優先にする。hotplug 後に Rift が崩れたら `Alt+Ctrl+Shift+R` で Rift service を restart し、`scrolling` 自体が不安定なときは `Alt+b` で一時的に `bsp` へ戻す
